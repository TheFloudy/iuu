...
module.exports = {
  info: {
    ...
    usage: "[URL-адрес YouTube | название песни]",
    aliases: ["p"],
  },
  ...
  run: async function (client, message, args) {
    ...
    if (!searchString) return sendError("я немогу начать!", message.channel);
    ...
    const play = async (song) => {
      ...
      if (!song) {
        if (!online.afk) {
          sendError("Покидаю голосовой канал, потому что думаю, что в очереди нет песен. Если вам нравится бот, оставайтесь 24/7 в голосовом канале run`! afk`", message.channel)
          message.guild.me.voice.channel.leave();////Если вы хотите, чтобы ваш бот оставался в гс 24/7, удалите эту строку :D
      ...
      let thing = new MessageEmbed()
        ...
        .setThumbnail(song.img)
        .setColor("BLUE")
        .addField("Название", song.title)
        .addField("Продолжительность", song.duration)
        .addField("По запросу", song.req.tag)
        .setFooter(`Просмотры: ${song.views} | ${song.ago}`)
